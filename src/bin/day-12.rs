...
impl Arrangement {
    pub fn count_by_skipping_contiguous_blocks(&self) -> usize {
        let mut cache = std::collections::HashMap::new();
        Self::count_valid_arrangments(&self.seq, &self.groups, &mut cache)
    }

    pub fn count_valid_arrangments(
        arrangement: &[u8],
        contiguous_blocks: &[usize],
        cache: &mut std::collections::HashMap<(Vec<u8>, Vec<usize>), usize>,
    ) -> usize {
        ...
        if arrangement[0] == b'#' || arrangement[0] == b'?' {
            ...
            let block_contains_only_damaged_strings = arrangement
                [..damaged_springs_in_this_block.min(arrangement.len())]
                .iter()
                .all(|&c| c == b'#' || c == b'?');
            ...
            && (
                ...
                if damaged_springs_in_this_block < springs_left {
                    result += Self::count_valid_arrangments(
                        &arrangement[damaged_springs_in_this_block + 1..],
                        &contiguous_blocks[1..],
                        cache,
                    );
                }
